...
public class KhachHangDAO implements DAOInterface<KhachHang>{
	...
	public ArrayList<KhachHang> selectAll() {
		...
		try {
			...
			while (rs.next()) {
				...
				Date thoiGianHieuLucCuaMaXacThuc = new Date(rs.getTimestamp("thoigianhieuluccuamaxacthuc").getTime());
				boolean trangThaiXacThuc = rs.getBoolean("trangthaixacthuc");
				String duongDanAnh = rs.getString("duongdananh");

				KhachHang kh = new KhachHang(maKhacHang, tenDangNhap, matKhau, hoVaTen, gioiTinh, diaChi, diaChiNhanHang, diaChiMuaHang, ngaySinh, soDienThoai, email, dangKyNhanBangTin, maXacThuc, thoiGianHieuLucCuaMaXacThuc, trangThaiXacThuc, duongDanAnh);
				ketQua.add(kh);
			}
	...
	public KhachHang selectById(KhachHang t) {
		...
		try {
			...
			while (rs.next()) {
				...
				ketQua = new KhachHang(maKhacHang, tenDangNhap, matKhau, hoVaTen, gioiTinh, diaChi, diaChiNhanHang, diaChiMuaHang, ngaySinh, soDienThoai, email, dangKyNhanBangTin, maXacThuc, thoiGianHieuLucCuaMaXacThuc, trangThaiXacThuc, duongDanAnh);
	...
	public int insert(KhachHang t) {
		...
		try {
			...
			st.setInt(12, t.isDangKyNhanBangTin()?1:0);
	...
	public int updateVerifyInformation(KhachHang t) {
		...
		try {
			...
			// Bước 2: tạo ra đối tượng statement
			String sql = "UPDATE khachhang " + " SET " + " maxacthuc = ?" + ", thoigianhieuluccuamaxacthuc=?" + ", trangthaixacthuc=?"
			+ " WHERE makhachhang=?";

			PreparedStatement st = con.prepareStatement(sql);
			st.setString(1, t.getMaXacThuc());
			java.util.Date  utilDate = new java.util.Date(t.getThoiGianHieuLucCuaMaXacThuc().getTime());
			st.setTimestamp(2, new Timestamp(utilDate.getTime()));
	...
	public KhachHang selectByTenDangNhap(String tenNguoiDung) {
		KhachHang ketQua = null;
		try {
			// Bước 1: tạo kết nối đến CSDL
			Connection con = JDBCUtil.getConnection();
